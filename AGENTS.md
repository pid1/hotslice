...
- Uvicorn is started with `proxy_headers=True` and `forwarded_allow_ips="*"` so that reverse proxies (Railway, etc.) can pass `X-Forwarded-Proto` and `X-Forwarded-For` headers. Without this, Starlette's trailing-slash redirect from `/mcp` → `/mcp/` uses `http://` instead of `https://`, causing 421 Misdirected Request errors on HTTPS edges. Do not remove these settings when deploying behind a reverse proxy.
