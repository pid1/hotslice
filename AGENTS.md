...
- Uvicorn is started with `proxy_headers=True` and `forwarded_allow_ips="*"` so that reverse proxies (Railway, etc.) can pass `X-Forwarded-Proto` and `X-Forwarded-For` headers. Do not remove these settings when deploying behind a reverse proxy.
